...
const templateWithJQuery = () => {
  ...
  for (let i = 0; i < starWarsPeople.length; i++) {
    let sec = $($('section').html());
    sec($('h2').text(starWarsPeople.name));
    sec($('h3').text(starWarsPeople.height));
    sec($('p').text(starWarsPeople.eye_color));
    $('main').append(sec);
  }
}
...
const listFoods = (recipe) => {
  ...
  ingreds.forEach( ingreds => {
    let wordOne = ingreds.slice(ingreds.indexOf(' ')+1);
    result.push(wordOne.slice(wordOne.indexOf(' ')+1));
  });
  return result;
};
